"""OneMgAdapter adapter — speaks the mock's RESTful, camelCase dialect."""
...
def _ok(data: dict) -> dict:
    if data.get("status") not in (200, 201):
        raise RetailerError(f"onemg: {data.get('message', 'unknown error')}")
    return data

class OneMgAdapter(RetailerAdapter):
    description = "Tata 1mg — pharmacy, medicines, supplements, healthcare, ayurveda, baby care"

    intent_fields = {'prescription_required': '<boolean, true if user asks for prescription drugs, false otherwise>'}

    enhance_spec = {'field': 'pack_size', 'fill_as': 'pack_size_options', 'ask': 'what pack sizes is this medicine available in?', 'example': '{"1MG-001": ["10 tablets", "30 tablets"]}'}
    ...
    async def search(self, intent: dict) -> tuple[dict, list[dict]]:
        ...
        async with self.client() as client:
            resp = await client.get(f"/onemg/api/v2/products/search", params=params)
