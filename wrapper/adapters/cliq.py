"""CliqAdapter adapter — speaks the mock's RESTful, camelCase dialect."""
...
def _ok(data: dict) -> dict:
    if data.get("status") not in (200, 201):
        raise RetailerError(f"cliq: {data.get('message', 'unknown error')}")
    return data

class CliqAdapter(RetailerAdapter):
    description = "Tata CLiQ — premium lifestyle, fashion, luxury, apparel, footwear, accessories"

    intent_fields = {'color': '<string, only if user specifies a color like Black, Silver>'}

    enhance_spec = {'field': 'material', 'fill_as': 'material_options', 'ask': 'what materials is this lifestyle product made of?', 'example': '{"CLQ-001": ["Leather", "Synthetic"]}'}
    ...
    async def search(self, intent: dict) -> tuple[dict, list[dict]]:
        ...
        async with self.client() as client:
            resp = await client.get(f"/cliq/api/v2/products/search", params=params)
