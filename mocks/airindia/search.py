"""Airindia product search + detail (REST style: GET /products/search)."""
...
@router.get("/airindia/api/v2/products/search")
def search(
    text: str,
    ...
    hits = []
    for p in DATA:
        haystack = f"{p['name']} {p.get('brandName', '')} {p['category']}".lower()
...
@router.get("/airindia/api/v2/products/{code}")
def detail(code: str):
    for p in DATA:
        if p["code"] == code:
