"""OneMg order operations: place an order from a cart, look one up."""
...
@router.post("/onemg/api/v2/orders")
async def place_order(req: OrderRequest):
    ...
    order_id = f"1MGORD-{uuid.uuid4().hex[:8].upper()}"
    view = cart_view(cart)
    ORDERS[order_id] = {
        "orderId": order_id,
...
@router.get("/onemg/api/v2/orders/{order_id}")
