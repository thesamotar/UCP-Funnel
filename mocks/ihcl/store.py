"""Shared state, catalog data, and helpers for the mock Ihcl service.

Ihcl's "own database" is its trio of Supabase tables (ihcl_products /
...
async def load() -> None:
    ...
    client = await db()
    DATA.clear()
    for row in (await client.table("ihcl_products").select("native,image_url").execute()).data:
        native = row["native"]
        if row.get("image_url"):
            native["imageUrl"] = row["image_url"]
        DATA.append(native)
    if not DATA:
        raise RuntimeError("ihcl_products is empty — run `python -m db.seed`")
    CARTS.clear()
    for row in (await client.table("ihcl_carts").select("id,payload").execute()).data:
        CARTS[row["id"]] = row["payload"]
    ORDERS.clear()
    for row in (await client.table("ihcl_orders").select("id,payload").execute()).data:
        ORDERS[row["id"]] = row["payload"]
    print(f"[ihcl] loaded {len(DATA)} products, {len(CARTS)} open carts, "
...
async def persist_cart(cart: dict) -> None:
    await (await db()).table("ihcl_carts").upsert(
        {"id": cart["cartId"], "payload": cart, "updated_at": _now()}).execute()


async def drop_cart(cart_id: str) -> None:
    await (await db()).table("ihcl_carts").delete().eq("id", cart_id).execute()


async def persist_order(order: dict) -> None:
    await (await db()).table("ihcl_orders").upsert(
